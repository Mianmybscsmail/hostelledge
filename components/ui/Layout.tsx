...
import { Home, List, Utensils, Users, Settings, PlusCircle, ShoppingCart } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, isAdmin, canEdit }) => {
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-red-500/30">
      <main className="pb-24 max-w-md mx-auto min-h-screen bg-zinc-950 shadow-2xl overflow-x-hidden relative">
        {children}
      </main>

      {/* Sticky Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 bg-zinc-900/95 backdrop-blur-md border-t border-zinc-800 z-50">
        <div className="max-w-md mx-auto flex justify-between items-center px-2 py-3">
          ...
          <button
            ...
            className={`flex flex-col items-center p-2 rounded-xl transition-all ${
              activeTab === 'meals' ? 'text-red-500 scale-105' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
           ...
           {(isAdmin || canEdit) && (
            ...
            >
              ...
              <span className="text-[10px] mt-1 font-medium text-zinc-400">Add</span>
            </button>
          )}
          
          {/* If viewer, show Expenses instead of Add in the middle if desired, but sticking to requested tabs */}
          {!isAdmin && !canEdit && (
             <button
             onClick={() => setActiveTab('market')}
             className={`flex flex-col items-center p-2 rounded-xl transition-all ${
               activeTab === 'market' ? 'text-red-500 scale-105' : 'text-zinc-500 hover:text-zinc-300'
             }`}
           >
          ...
          <button
            ...
            className={`flex flex-col items-center p-2 rounded-xl transition-all ${
              activeTab === 'friends' ? 'text-red-500 scale-105' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center p-2 rounded-xl transition-all ${
              activeTab === 'settings' ? 'text-red-500 scale-105' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
